...
export const api = {
  ...
  addWatchlistStock: (id: number, symbol: string) => request(`/watchlists/${id}/stocks/${encodeURIComponent(symbol)}`, { method: 'POST' }, true),
  removeWatchlistStock: (id: number, symbol: string) => request(`/watchlists/${id}/stocks/${encodeURIComponent(symbol)}`, { method: 'DELETE' }, true),
  ...
  trade: (side: 'buy' | 'sell', payload: { symbol: string; quantity: string; price: string; fees: string; notes?: string; executed_at?: string }) => request(`/portfolio/${side}`, { method: 'POST', body: JSON.stringify(payload) }, true),
}
