...
export function TradeDialog({ side, symbol, exchange, currency, marketPrice, owned, onClose, onComplete }: { side: 'buy' | 'sell'; symbol: string; exchange?: string | null; currency?: string; marketPrice?: string; owned?: string; onClose: () => void; onComplete: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setLoading(true)
    try { await api.trade(side, { symbol, exchange, quantity, price, fees, notes: notes || undefined, executed_at: `${date}T00:00:00Z` }); onComplete() } catch (err) { setError(err instanceof ApiError ? err.message : 'Unable to record transaction.') } finally { setLoading(false) }
  }
  const label = side === 'buy' ? 'Buy' : 'Sell'
  return <div className="modal-backdrop" role="presentation"><form className="modal trade-record-modal" onSubmit={submit}><button type="button" className="close" onClick={onClose}>×</button><p className="eyebrow">Record {label.toLowerCase()}</p><h2>{symbol}{exchange ? ` · ${exchange}` : ''}</h2>{marketPrice && <p className="muted">Current market price: <strong>{money(marketPrice, currency)}</strong> <button type="button" className="text-button" onClick={() => setPrice(marketPrice)}>Use current price</button></p>}{owned && <p>Owned: <strong>{number(owned)} shares</strong></p>}<label>Quantity<input autoFocus inputMode="decimal" value={quantity} onChange={(event) => setQuantity(event.target.value)} /></label><label>Actual {label.toLowerCase()} price<input inputMode="decimal" placeholder="0.00" value={price} onChange={(event) => setPrice(event.target.value)} /></label><div className="trade-form-grid"><label>Fees (optional)<input inputMode="decimal" value={fees} onChange={(event) => setFees(event.target.value)} /></label><label>Transaction date<input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label></div><label>Notes (optional)<input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Broker, account, or trade note" /></label><p className="estimate">Recorded {side === 'buy' ? 'cost' : 'proceeds'}: <strong>{money(total, currency)}</strong></p>{error && <p className="form-error">{error}</p>}<button className="primary" disabled={loading}>{loading ? 'Saving…' : `Record ${label}`}</button></form></div>
}
